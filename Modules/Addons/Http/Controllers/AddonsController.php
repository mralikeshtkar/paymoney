...
class AddonsController extends Controller
{
    ...
    public function upload(UploadAddonRequest $request)
    {
        try {
            ...
            $supportedVersions = config($addon->get('alias') . '.' . 'supported_versions');

            $appVexplode = explode('.', $appVersion);
            if (count($appVexplode) > 2) {
                $appVersion = implode('.', [$appVexplode[0], $appVexplode[1]]);
            }

            $lastSversion = end($supportedVersions);
            if (str_contains($lastSversion, '^')) {
                $lastSversion = str_replace('^', '', $lastSversion);
                $this->getLastSupportedVersion($lastSversion);
            } else {
                $this->getLastSupportedVersion($lastSversion);
            }

            if (!in_array($appVersion, $supportedVersions) && !($appVersion >= $lastSversion)) {
    ...
    function getLastSupportedVersion($lastSversion)
    {
        $lastSversionExplode = explode('.', $lastSversion);
        if (count($lastSversionExplode) > 2) {
            $lastSversion = implode('.', [$lastSversionExplode[0], $lastSversionExplode[1]]);
        }
    }
