...
use App\Exceptions\Api\V2\SendMoneyException;
...
class Transfer extends Model
{
    ...
    public function mailToSender($transfer, $arr)
    {
        if (checkAppMailEnvironment())
        {
            ...
            if (!empty($sender_info->subject) && !empty($sender_info->body))
            {
                ...
                $sender_msg     = str_replace('{sender_id}', getColumnValue($transfer->sender), $sender_info->body);
            }
            else
            {
                $sender_subject = $englishSenderLanginfo->subject;
                $sender_msg     = str_replace('{sender_id}', getColumnValue($transfer->sender), $englishSenderLanginfo->body);
            }
            $sender_msg = str_replace('{amount}', moneyFormat(optional($transfer->currency)->symbol, formatNumber($arr['amount'])), $sender_msg);
            ...
            $sender_msg = str_replace('{fee}', moneyFormat(optional($transfer->currency)->symbol, formatNumber($arr['p_calc'] + $arr['charge_fixed'])), $sender_msg);
        ...
        if (checkAppMailEnvironment())
        {
            ...
            if (isset($arr['userInfo']))
            {
                if (!empty($receiver_info->subject) && !empty($receiver_info->body))
                {
                    ...
                    $receiver_msg     = str_replace('{receiver_id}', getColumnValue($transfer->receiver), $receiver_info->body); //
                }
                else
                {
                    $receiver_subject = $englishLangReceiverinfo->subject;
                    $receiver_msg     = str_replace('{receiver_id}', getColumnValue($transfer->receiver), $englishLangReceiverinfo->body); //
                }
                $receiver_msg = str_replace('{amount}', moneyFormat(optional($transfer->currency)->symbol, formatNumber($arr['amount'])), $receiver_msg);
                $receiver_msg = str_replace('{sender_id}', getColumnValue($transfer->sender), $receiver_msg);
            ...
            else
            {
                ...
                $subject                       = __('Notice of Transfer');
                $message                       = __('Hi :x,', ['x' => $unregisteredUserNameFromEmail]) . '<br><br>';
                $message .= __('You have got :x money transfer from :y.', ['x' => moneyFormat(optional($transfer->currency)->symbol, formatNumber($arr['amount'])), 'y' => optional($transfer->sender)->email]) . '<br>';
                $message .= __('To receive, please register on : :x with current email.', ['x' => url('/register')]) . '<br><br>';
        ...
        if (checkAppSmsEnvironment())
        {
            if (!empty($transfer->sender->carrierCode) && !empty($transfer->sender->phone))
            {
                ...
                if (!empty($senderSmsInfo->subject) && !empty($senderSmsInfo->body))
                {
                    ...
                    $sender_msg     = str_replace('{sender_id}', getColumnValue($transfer->sender), $senderSmsInfo->body);
                }
                else
                {
                    $sender_subject = $englishSenderLangSMSinfo->subject;
                    $sender_msg     = str_replace('{sender_id}', getColumnValue($transfer->sender), $englishSenderLangSMSinfo->body);
                }
                $sender_msg = str_replace('{amount}', moneyFormat(optional($transfer->currency)->symbol, formatNumber($arr['amount'])), $sender_msg);

                try {
                    sendSMS(optional($transfer->sender)->carrierCode . optional($transfer->sender)->phone, $sender_msg);
                    return [
                        'ex' => null,
                    ];
                }
        ...
        if (checkAppSmsEnvironment())
        {
            if (!empty($transfer->receiver->carrierCode) && !empty($transfer->receiver->phone))
            {
                ...
                if (!empty($receiverSmsInfo->subject) && !empty($receiverSmsInfo->body))
                {
                    ...
                    $receiver_msg     = str_replace('{receiver_id}', getColumnValue($transfer->receiver), $receiverSmsInfo->body); //
                }
                else
                {
                    $receiver_subject = $englishLangReceiverSmsInfo->subject;
                    $receiver_msg     = str_replace('{receiver_id}', getColumnValue($transfer->receiver), $englishLangReceiverSmsInfo->body); //
                }
                $receiver_msg = str_replace('{amount}', moneyFormat(optional($transfer->currency)->symbol, formatNumber($arr['amount'])), $receiver_msg);
                $receiver_msg = str_replace('{sender_id}', getColumnValue($transfer->sender), $receiver_msg);

                try {
                    sendSMS(optional($transfer->receiver)->carrierCode . optional($transfer->receiver)->phone, $receiver_msg);
                    return [
                        'ex' => null,
                    ];
                }
    ...
    {
        $message = __('You have received :x from :y.', ['x' => moneyFormat(optional($transfer->currency)->symbol, formatNumber($arr['amount'])), 'y' => getColumnValue($transfer->sender)]) . '<br><br>';
        try
        {
