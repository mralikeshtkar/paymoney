...
class RequestPayment extends Model
{
    ...
    public function mailToRegisteredRequestAcceptor($requestPayment, $arr)
    {
        ...
        if (checkAppMailEnvironment())
        {
            ...
            $req_create_msg = str_replace('{creator}', getColumnValue($requestPayment->user), $req_create_msg);
            $req_create_msg = str_replace('{amount}', moneyFormat(optional($requestPayment->currency)->symbol, formatNumber($arr['amount'])), $req_create_msg);
    ...
    public function mailToUnRegisteredRequestAcceptor($requestPayment, $arr)
    {
        ...
        if (checkAppMailEnvironment()) {
            ...
            $subject                       = __('Notice of Request Creation!');
            $message                       = __('Hi :x,', ['x' => $unregisteredUserNameFromEmail]) . '<br><br>';
            $message .= __('You have got :x payment request from :y.', ['x' => moneyFormat(optional($requestPayment->currency)->symbol, formatNumber($arr['amount'])), 'y' => $arr['senderEmail']]) . '<br>';
            $message .= __('To accept the request, please register on : :x with current email.', ['x' => url('/register')]) .'<br><br>';
    ...
    public function smsToRequestAcceptor($requestPayment, $arr)
    {
        if (checkAppSmsEnvironment())
        {
            if (isset($arr['userInfo']))
            {
                ...
                $reqSmsTempInfo_msg = str_replace('{amount}', moneyFormat(optional($requestPayment->currency)->symbol, formatNumber($arr['amount'])), $reqSmsTempInfo_msg);
                $reqSmsTempInfo_msg = str_replace('{creator}', getColumnValue($requestPayment->user), $reqSmsTempInfo_msg);
            ...
            else
            {
                ...
                $message = __('Amount :x has been requested by :y.', ['x' => moneyFormat(optional($requestPayment->currency)->symbol, formatNumber($arr['amount'])), 'y' => getColumnValue($requestPayment->user)]). '<br><br>';
                try
                {
