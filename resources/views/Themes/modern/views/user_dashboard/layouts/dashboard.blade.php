                ...
                <div class="col-md-7 border-right p-4">
                    ...
                    @php
                        if (!empty($lastTransaction)) {
                            if (optional($lastTransaction->transaction_type)->name == 'Transferred') {
                                $transactionName = 'Money Transfer';
                            } elseif (optional($lastTransaction->transaction_type)->name == 'Received') {
                                $transactionName = 'Money Received';
                            } elseif (optional($lastTransaction->transaction_type)->name == 'Exchange_From' || optional($lastTransaction->transaction_type)->name == 'Exchange_To') {
                                $transactionName = 'Money Exchange';
                            } elseif (optional($lastTransaction->transaction_type)->name == 'Request_From' || optional($lastTransaction->transaction_type)->name == 'Request_To') {
                                $transactionName = 'Request Money';
                            } else {
                                if (str_contains(optional($lastTransaction->transaction_type)->name, '_')) {
                                    $transactionName = str_replace('_', ' ', optional($lastTransaction->transaction_type)->name);
                                } else {
                                    $transactionName = optional($lastTransaction->transaction_type)->name;
                                }
                            }
                        }
                    @endphp
                    ...
                    @else
                        @if (!empty($lastTransaction))
                        <h5>{{ __('Your last transaction was') }} <span class="text-primary">{{ formatNumber($lastTransaction->subtotal, optional($lastTransaction->currency)->id) }} {{ optional($lastTransaction->currency)->code }} </span> <span style="font-size:small;">( {{ $transactionName }} )</span> </h5>
                        @else
                                                            ...
                                                            @if($transaction->transaction_type_id == Deposit)
                                                                @if($transaction->subtotal > 0)
                                                                    <td class="text-right pr-5">
                                                                        <p>
                                                                            <span class="text-16 font-weight-600"> +{{ formatNumber($transaction->subtotal, optional($transaction->currency)->id) }}</span>
                                                                            <span class="c-code">({{ optional($transaction->currency)->code }})</span>
                                                                        </p>
                                                                    </td>
                                                                @endif
                                                            @elseif($transaction->transaction_type_id == Withdrawal)
                                                                                        ...
                                                                                        <div class="col-md-12 col-sm-12 mt-5">
                                                                                            <div class="text-center mb-2">
                                                                                                @if( $transaction->transaction_type_id == Payment_Sent && $transaction->status == 'Success' && !isset($transaction->dispute->id))
                                                                                                    <a id="dispute_{{$transaction->id}}" href="{{url('/dispute/add/').'/'.$transaction->id}}" class="btn btn-primary btn-sm">{{ __('Open Dispute') }}</a>
                                                                                                @endif
                                                                                            </div>
                                                                                        </div>
                                                                                </div>
                                        ...
                                        <div class="d-flex align-items-center" >
                                            <div class="w-100">
                                                <h4 class="text-18 font-weight-600">
                                                    <span>{{ '+'.formatNumber($wallet->balance, optional($wallet->currency)->id) }}</span>
                                                </h4>
                                                <p class="side-text my-0 ml-2">
                                                    @if(optional($wallet->currency)->type == 'fiat' && $wallet->is_default == 'Yes')
                                                        <span>{{ optional($wallet->currency)->code }}&nbsp;<span class="badge badge-secondary">{{ __('Default') }}</span></span>
                                                    @else
